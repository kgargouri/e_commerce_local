...
def main():
    try:
        spark = SparkSession.builder.appName("TransformProducts").getOrCreate()

        # Reading the CSV
        run_date = sys.argv[1]  # retrieves {{ ds }}
        input_path = f"/tmp/products/extracted/products_{run_date}.csv"
        output_path = f"/tmp/products/transformed/products_transform_{run_date}.csv"
        output_dir = f"/tmp/products/transformed/products_transform_{run_date}"
