...
def main():
    try:
        spark = SparkSession.builder.appName("TransformUsers").getOrCreate()

        # Reading the CSV
        run_date = sys.argv[1]  # retrieves {{ ds }}
        input_path = f"/tmp/users/extracted/users_{run_date}.csv"
        output_path = f"/tmp/users/transformed/users_transform_{run_date}.csv"
        output_dir = f"/tmp/users/transformed/users_transform_{run_date}"
        ...
        # transformation
        df_clean = df.drop("eyeColor")
        df_clean = df_clean.drop("hair")
